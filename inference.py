...
#     mask_rgb[np.all(mask_convert == 3, axis=0)] = [0, 128, 128]
...
#     mask_rgb[np.all(mask_convert == 4, axis=0)] = [128, 0, 0]
#     return mask_rgb
...
def label2rgb(mask):
    ...
    mask_rgb = np.zeros(shape=(h, w, 3), dtype=np.uint8)
    mask_convert = mask[np.newaxis, :, :]
    mask_rgb[np.all(mask_convert == 3, axis=0)] = [0, 0, 168]
    mask_rgb[np.all(mask_convert == 0, axis=0)] = [0, 0, 0]
    mask_rgb[np.all(mask_convert == 1, axis=0)] = [75, 217, 86]
    mask_rgb[np.all(mask_convert == 2, axis=0)] = [255, 223, 115]
    mask_rgb[np.all(mask_convert == 4, axis=0)] = [178, 178, 178]
    mask_rgb[np.all(mask_convert == 5, axis=0)] = [190, 235, 255]
    return mask_rgb
