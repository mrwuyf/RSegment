...
class ISNetLoss(nn.Module):
    def __init__(self, ignore_index=255, weight=0.4):
        ...
        self.main_loss = self.main_loss = JointLoss(SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index),
                                                    DiceLoss(smooth=0.05, ignore_index=ignore_index), 1.0, 1.0)
        self.aux_loss = SoftCrossEntropyLoss(smooth_factor=0.05, ignore_index=ignore_index)
        self.weight = weight
    ...
    def forward(self, logits, labels):
        if self.training:
            logits_main, logits_aux = logits
            loss = self.main_loss(logits_main, labels) + 0.4 * self.aux_loss(logits_aux, labels)
        else:
            loss = self.main_loss(logits, labels)
